...
"""
    ...
    def scrape_huggingface_models(self, limit: int = 50) -> List[dict]:
        try:
            self.log("Scraping models from HuggingFace...", "INFO")
            url = "https://huggingface.co/api/models"
            # API HuggingFace tidak memerlukan parameter sort/direction
            response = requests.get(url, timeout=30)
            response.raise_for_status()
            models_data = response.json()
            models = []
            # Ambil hanya sejumlah limit yang diminta
    ...
    def scrape_huggingface_datasets(self, limit: int = 50) -> List[dict]:
        try:
...
def main():
    ...
    # Scrape from HuggingFace
    print(f"\n{Fore.CYAN}============================================================{Style.RESET_ALL}")
    bot.log("Scraping data from HuggingFace...", "INFO")
    print(f"{Fore.CYAN}============================================================{Style.RESET_ALL}")
    
    all_models = bot.scrape_huggingface_models(limit=50)
    all_datasets = bot.scrape_huggingface_datasets(limit=50)
