        ...
        try:
            ...
            if response.status_code != 200:
                try:
                    ...
                    print(f"{Colors.ERROR}│  └─ ✗ Server error: {error_msg[:100]}{Colors.RESET}")
                except:
                    print(f"{Colors.ERROR}│  └─ ✗ HTTP {response.status_code}: {response.text[:100]}{Colors.RESET}")
                return None
            return response.json()
        except requests.exceptions.RequestException as e:
            print(f"{Colors.ERROR}│  └─ ✗ Request error: {str(e)[:100]}{Colors.RESET}")
            return None
        except Exception as e:
            print(f"{Colors.ERROR}│  └─ ✗ Training error: {str(e)[:100]}{Colors.RESET}")
            return None
    ...
    def process_account(self, private_key: str, index: int, total: int, models: list, datasets: list) -> bool:
        try:
            ...
            for i, model in enumerate(models, 1):
                ...
                training_response = self.submit_training(token, model)
            ...
            for i, dataset in enumerate(datasets, 1):
                ...
                training_response = self.submit_training(token, dataset)
                if training_response:
